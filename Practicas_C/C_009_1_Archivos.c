...
* FILE * fopen (const char *filename, const char *opentype);
    ...
    "r+b o rb+" : Abre un archivo en modo binario para actualizacion (lectura y escritura).
...
Esta funcion lee un bloque de una "stream" de datos. Efectúa la lectura de un arreglo de elementos "count", cada uno de los cuales tiene un taman-io definido por "size". Luego los guarda en el bloque de memoria especificado por "ptr". El indicador de posicion de la cadena de caracteres avanza hasta leer la totalidad de bytes. Si esto es exitoso la cantidad de bytes leídos es (size*count).


PARAMETROS:
ptr : Puntero a un bloque de memoria con un taman-io mínimo de (size*count) bytes.
size : Taman-io en bytes de cada elemento (de los que voy a leer).
count : Número de elementos, los cuales tienen un taman-io "size".
stream: Puntero a objetos FILE, que especifica la cadena de entrada.
*
* fscanf
La funcion fscanf funciona igual que scanf en cuanto a parámetros, pero la entrada se toma de un fichero en lugar del teclado.
...
Esta funcion sirve para poder cerrar un fichero que se ha abierto.
